...
// Define select function in your file
...
function treat_current_path(&$selected_files, &$dir_path, $select_function_type, $recursion_level, $maximum_recursion_level, &$counter=NULL, &$maximum_number_selected_files) {
	...
	foreach($dir_path as $cur_dir) {
		...
		while (false !== ($cur_file = readdir($hdir))) {
			...
			$file_exten='';
		        if( strlen($file_attributes['name'])>3 && substr($file_attributes['name'], -4, 1)=='.' ) {
                		$file_exten=substr( $file_attributes['name'], -3, 3);
        		}
...
function write_log($message, $log_path, $prefix_of_line='') {
	...
	if ($fp) {
		...
		if( count($message)===1 && array_key_exists('log', $message) && $message['log']==='blank_line' ) {
			...
			$message_text=$prefix_of_line;

			if( strlen($message_text)>0 ) $message_text.=' ';
...
function action_ftp_put($files_to_copy, $dir_destination, $log_file='', $mode="FTP_BINARY", $passive_mode=true) {
	...
	if( $function_result ) {
		...
		$ftp_connection=ftp_connect($destination_params['host'], $ftp_port);
